"""Import all Héraðsdómstólar verdicts from island.is GraphQL into lausnir_v2.

Queries all 8 district courts in a single paginated batch using slug identifiers.
Court differentiation is stored in documents.court via graphql_to_abbreviation().
"""
...
# All 8 district court slugs — these are the identifiers the island.is API uses
# when filtering by district court. Court names ("Héraðsdómur Reykjavíkur" etc.)
# are returned per-item in the `court` field and mapped in court_names.py.
_DISTRICT_COURT_SLUGS = [
    "hd-reykjavik",
    "hd-reykjanes",
    "hd-vesturland",
    "hd-vestfirdir",
...
_CHECKPOINT_FILE = _CHECKPOINT_DIR / "heradsdomstolar.json"
...
async def _fetch_list_page(client: httpx.AsyncClient, page: int) -> dict:
    """Fetch one page (10 items) of all 8 district courts combined."""
    payload = {
        "query": _GQL_QUERY,
        "variables": {"input": {"court": _DISTRICT_COURT_SLUGS, "page": page}},
    }
    for attempt in range(3):
        data = await post_with_retry(client, _GQL_ENDPOINT, payload)
        verdicts = (data.get("data") or {}).get("webVerdicts")
        if verdicts is not None:
            return verdicts
        gql_errors = data.get("errors", [])
        if attempt < 2:
            log.warning(
                "GraphQL data:null on page %d (errors: %s) — retry %d/3",
                page, gql_errors, attempt + 1,
            )
            await asyncio.sleep(5.0)
        else:
            raise ValueError(f"GraphQL data:null for page {page} after 3 attempts: {gql_errors}")
...
async def _fetch_detail(
    ...
    """Fetch pdfString and supplementary fields via Next.js JSON route."""
...
def _render_and_save(doc: Document, config: SourceConfig) -> Path | None:
    """Write .md to disk and save PDF bytes. Returns markdown Path or None on failure."""
...
async def main(limit: int | None = None) -> None:
    ...
    with Progress(
        ...
        task_id = progress.add_task("Importing heradsdomstolar…", total=None)
        ...
        async with make_client() as client:
            ...
            while True:
                ...
                try:
                    ...
                    log.error("Skipping page %d — list fetch failed: %s", page, exc)
                    _save_checkpoint(page, last_page or 0, imported_count)
                    page += 1
                    continue
                ...
                async with _db_conn.AsyncSessionLocal() as session:
                    for doc in docs:
                        try:
                            ...
                            log.error("Upsert failed for %s: %s", doc.external_id, exc)
